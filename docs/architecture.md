...
输出处理（v1.2）：不按字数裁剪，只去除外围 CR/LF；保留连接空格，拒绝内部控制字符、明确上下文回声、工具调用和未完成的 finish reason。请求预算 4096 tokens、90 秒；最多接收 8192 个 UTF-16 单元，超过资源预算整条拒绝并提示，不截断插入、不自动重试。完整内容通过 IPC 传给插入器；长预览可翻页，采纳内容不受可见范围影响。
