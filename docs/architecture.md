...
模型统一选用 DeepSeek 官方 `deepseek-flash`，思考深度默认自动，可选 Max 或关闭，文本和截图使用同一个 API Key。官方当前模型表已列出图像理解支持，Chat Completions 文档提供图片内容块；原需求中的截图能力疑问在文档层面已解决，仍须做真实请求验收。[模型能力](https://api-docs.deepseek.com/zh-cn/quick_start/pricing/)、[请求格式](https://api-docs.deepseek.com/zh-cn/api/create-chat-completion/)
...
v1.4 起手动截图仅在启用时生效；暂停时隐藏浮窗，启动或解锁后不会自动截图。锁屏、密码目标及无输入位置时拒绝手动截图生成。截图只属于当前 RequestId，后续续写不再附带图片。无选区采纳后按现有规则继续生成下一条；有选区时采纳复制后停止对同一选区连续生成。
...
输出处理（v1.3）：先要求模型返回 JSON 对象，严格校验 `status` 和 `text` 字段。只有 `status=ok` 的正文进入质量校验；`insufficient_context`、`cannot_continue`、空结果、异常格式、常见拒绝／道歉说明及中断响应只映射为程序内状态，不展示或采纳原始回复。不按字数裁剪，只去除外围 CR/LF；保留连接空格，拒绝内部控制字符、明确上下文回声、工具调用和未完成的 finish reason。不思考模式预算 4096 tokens、90 秒；自动和 Max 使用服务默认生成预算，超时分别为 300 秒和 900 秒；最多接收 8192 个 UTF-16 单元，超过资源预算整条拒绝并提示，不截断插入、不自动重试。完整内容通过 IPC 传给插入器；长预览可翻页，采纳内容不受可见范围影响。
...
v1.3.1：SSE 可选字段为 null 或空数组时按未调用工具处理，真实工具调用仍拒绝；只从成功 JSON 提取正文，额外元数据忽略，重复关键字段拒绝。短语和半句话优先续写。文本接口暂未就绪时最多重读两次（每次 120 ms），前台变化即停止；诊断只记录固定阶段代码、模式及字符数。UIA 非空选区允许跨过焦点段落，要求与焦点范围相交且位于所属 TextPattern 文档范围内；段落级 TextPattern 无法容纳整个选区时，向上定位可容纳该选区的文本容器；光标模式仍限制在焦点控件的文字边界。
...
v1.4.2：Composing 失败不再将下一次检测永久设为 MaxValue；启用时在约 750 ms 后复查，暂停取消。UIA 组合范围必须包含文本、位于当前文档，并关联光标或与选区严格相交；与选区只接触边缘的旧范围不阻塞。真实活动组合输入没有基于等待时长的放行机制。

自动思考按任务选择：纯文本续写（包括长选区）为 low，截图为 high；显式 Max 为 max，不思考为 none。思考模式保留服务默认生成预算，正文资源限制与完整性规则不变。选区仍完整发送、光标上下文仍各 300 字符。提示词要求充分发展主题后收束，而非偏向一句总结；有后文时仍优先精确衔接，允许短补全。

生成期间使用独立 UI 计时器显示等待响应／思考／生成正文及耗时；阶段来自流中的 reasoning_content 和 content，展示前仍须完整 JSON、stop 与正文校验。CompletionTrace 只保留计数、耗时、枚举结束原因和服务 token 统计，不保存正文或思考字符串。finish_reason=length 单独映射 OutputLimitReached，拒绝展示部分结果；其他未完整结束仍为 IncompleteResponse。
